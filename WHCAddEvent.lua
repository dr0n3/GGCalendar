...
function WHCCalendar:OnBtnNewEventAccept()
	...
	if not error then
		local tEvent = self:NewEvent(name, attendees, WHCCalendar.tCurrentDate, {hour = hour, min = min})
...
function WHCCalendar:OnBtnNewEventCancel()
	self.wndAddEvent:Destroy()
	self.wndAddEvent = nil
	self.wndMain:Enable(true)
end