...
GGCalendar.tCurrentDate = {} -- { day, month, year }

function GGCalendar:NewEvent(strName, iAttendees, tDate, tTime)
...
function GGCalendar:OnCalendarFormShow(wndHandler, wndControl)
	local now = os.date("*t")
...
function GGCalendar:RefreshEventList()
...
function GGCalendar:OnBtnAddEvent()
...
function GGCalendar:OnCalendarItemClick(wndHandler, wndControl)
...
function GGCalendar:SetCurrentDate(day, month, year)	
	self.tCurrentDate = {day=day, month=month, year=year}
end

function GGCalendar:FindEventsOnDate(tDate)
